...
(function(jQuery, window, document) {
	...
	GvNIX_Loupe.prototype = {
		...
		/**
		 * This method add event click to the loupe button that displays the
		 * datatable Dialog
		 */
					...
					.click(function() {
						...
						buttonOpts[data.acceptlabel] = function() {
							...
							if (onAcceptNameFunction !== "") {
								...
								instance._fnAcceptItemCallback
										.add(callbackFunctions[onAcceptNameFunction]);
							}

							// Fire Callback function
							instance._fnAcceptItemCallback
									.fire(onAcceptNameFunction);
						...
						selectorDialog
								.load(
										data.controllerurl + "?selector&path="
												+ data.listpath + "&"
												+ baseFilters,
										function(a, e, i) {
											setTimeout(
													function() {
														...
														// Initialize Row Click
														// event
														datatableInstance
																.fnRowClick();

														// Filtering by current
														// value or searchField
														var currentValue;
		...
		/**
		 * This method add event keyup to the generated input to update the
		 * label with the correct data to display
		 */
			...
			$input.keyup(function(a) {
				...
				if (timeOutClear) {
					setTimeout(function() {
						var text = $input.val();
						if (text != "") {
							GvNIX_Loupe.prototype._fnFindRecordByAll(text,
									instance, sufix);
						} else {
							$container.find(
									".dropdown_div")
		...
		"_fnFindRecordByAll" : function(search, instance, sufix) {
			...
			jQuery.ajax({
				...
				success : function(object) {
					...
					instance._fnCreateDropDownList(input, object, data.pkfield,
							instance, sufix);
				},
					...
					.ajax({
						...
						success : function(element) {
							...
							if (onSetNameFunction !== "") {
								...
								instance._fnSetItemCallback
										.add(callbackFunctions[onSetNameFunction]);
							}
							input.val(object.__caption__);

							var inputValue = undefined;
							if (data.searchfield != undefined) {
								var searchFieldSplit = data.searchfield
										.split(",");
							...
							// Set initialized true because is necessary put
							// this
							// after first call to findById, because this
							// variable
		...
		"_fnFindRecordByField" : function(valueInput, instance, sufix,
			...
			if (searchField != "") {
				...
				if (additionalfieldsAux != "") {
					additionalfieldsAux = additionalfieldsAux + ","
							+ data.pkfield;
				} else {
			...
			$
					.ajax({
						...
						error : function(element) {
							var error = "";
							if (element.responseJSON != undefined
									|| element.responseJSON != null) {
								error = element.responseJSON[0].Error;
							} else if (element.responseText != undefined
									|| element.responseText != null) {
		...
		"_fnCreateDropDownList" : function(input, data, pkField, instance,
			...
			callbackFunctions = {};
			if (onSetNameFunction !== "") {
				callbackFunctions[onSetNameFunction] = function(functionName,
						item) {
					if (typeof window[onSetNameFunction] == "function") {
			...
			if (onDrawNameFunction !== "") {
				var itemToDraw = "";
				callbackFunctions[onDrawNameFunction] = function(functionName,
						item) {
				...
				instance._fnDrawItemCallback
						.add(callbackFunctions[onDrawNameFunction]);
			}
			...
			for (i in data) {
				...
				if (onDrawNameFunction !== "") {
					instance._fnDrawItemCallback
							.fire(onDrawNameFunction, [ i ]);
				} else {
					itemToDraw = "";
				}

				// Draw Caption by default
				var toDraw = "<span style='font-weight:bold;'>"
						+ item.__caption__ + "</span>";
	...
	/**
	 * Store of all instances that have been created of Loupe, so one can look
	 * up other (when there is need of a master)
	 *
	...
	/**
	 * #17932 metodo que borra instancias creadas al abrir un dialogo.
	 * Function to remove an Instance with given Id.
	 */
	GvNIX_Loupe.fnRemoveInstance = function(id) {
