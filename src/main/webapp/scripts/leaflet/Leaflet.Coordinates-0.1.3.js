 ...
 * This plugin has been modified for gvNIX.
 */
...
L.Control.Coordinates = L.Control.extend({
	options: {
		...
		markerProps: {},
		// Properties to display UTM coordinates
		displayUTM:{
			display: false,
	...
	onAdd: function(map) {
		...
		if (options.useLatLngOrder) {
			// Added CSS styles for uniformity with gvNIX
			ySpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputY = this._createInput("inputY coordinates-input", this._inputcontainer);
			xSpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputX = this._createInput("inputX coordinates-input", this._inputcontainer);
		} else {
			xSpan = L.DomUtil.create("span", "", this._inputcontainer);
			this._inputX = this._createInput("inputX coordinates-input", this._inputcontainer);
			ySpan = L.DomUtil.create("span", "", this._inputcontainer);
		...
		//Add Utm support.
		if(this.options.displayUTM.display){
			this._utmFrom = proj4.Proj(this.options.displayUTM.sourceSRID);
			this._utmTo = proj4.Proj(this.options.displayUTM.destSRID);
		}
	 ...
	 *	Modified for gvNIX (removed on default _handleSubmit).
	 */
	 ...
	 */
	_createCoordinateLabel: function(ll, utm) {
		...
		if(!utm){
			...
			x = L.Util.template(opts.displayUTM.labelTemplateXUTM, {
				x: this._getNumber(ll.x, opts, true)
			});

			y = L.Util.template(opts.displayUTM.labelTemplateYUTM, {
				y: this._getNumber(ll.y, opts, true)
			});
	 ...
	 */
	_getNumber: function(n, opts, utm) {
		var res;
		if (opts.useDMS && !utm) {
			...
			if(!utm){
				...
				//Round a return decimal numbers
				var value = L.Util.formatNum(n, 0);
	...
	collapse: function() {
		if (!this._showsCoordinates) {
			...
			if (this._marker) {
				...
				// Support to 'Remove point' i18n
				var _removeMarkerText = this._marker.options.removeMarkerText;
				if (_removeMarkerText === undefined) {
					_removeMarkerText = 'Remove';
				}
				close.innerHTML = _removeMarkerText;
				close.href = "#";
	...
	_update: function(evt) {
		...
		if (pos) {
			...
			if(this.options.displayUTM.display){
			  // transforming point coordinates between SRID
			  var p = proj4.toPoint([pos.lng,pos.lat]);
			  var utmPoint = proj4.transform(this._utmFrom, this._utmTo, p);
			  var formated = this._createCoordinateLabel(utmPoint, true);
			  this._label.innerHTML = formated +"&nbsp;&nbsp;"+ this._label.innerHTML;
			}
		}
	},
