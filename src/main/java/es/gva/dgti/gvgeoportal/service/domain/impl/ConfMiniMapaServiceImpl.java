...
public class ConfMiniMapaServiceImpl implements ConfMiniMapaService {
    ...
    /*
     * (non-Javadoc)
     * @see es.gva.dgti.gvgeoportal.service.domain.ConfMiniMapaService#findConfMiniMapasByGeoPortal(es.gva.dgti.gvgeoportal.domain.GeoPortal)
     */
    public ConfMiniMapa findConfMiniMapasByGeoPortal(GeoPortal geoPortal){
        ...
        confMiniMapaResult = ConfMiniMapa
                .findConfMiniMapasByGeoPortal(geoPortal);

        if (confMiniMapaResult != null
                && confMiniMapaResult.getResultList().size() > 0) {
